...
class BulkSeatCreate(BaseModel):
    ticket_type_id: int = Field(..., description="ID of the ticket type associated with these seats")
    is_available: bool = Field(default=True, description="Whether the seats are available for booking")
    quantity: int = Field(..., gt=0, description="Number of seats to create")
